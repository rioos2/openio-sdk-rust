...
impl<'a> SignedRequest<'a> {
    ...
    /// Called by `Requests` and determines which signature function to use.
    pub fn sign_url(&mut self, creds: &AwsCredentials) -> String {
        // NOTE: Check the bucket and path
        if self.endpoint.is_bucket_virtual {
            if self.bucket.contains(".") && !self.path.contains(&format!("/{}/", self.bucket)) {
                self.path = format!("/{}{}", self.bucket, self.path);
        ...
        self.build_v2_url(&creds)
    }

    fn build_v2_url(&mut self, creds: &AwsCredentials) -> String {        
        ...
        let can_headers = canonical_headers_v2(&self.headers);
        let can_resources = canonical_resources_v2(&self.bucket, &self.path, self.endpoint.is_bucket_virtual);
        ...
        if self.method == "PUT" {
            let url = format!("{}://{}{}{}",
                                     ...
                                     port_str,
                                     can_resources);
            sign_url =  format!("{}\"url\":\"{}\",\"date\":\"{}\",\"content_type\":\"{}\",\"authorization\":\"{}\"{}",
                        "{", 
                        url,
                        date_str,
                        content_type,
